...
class ParseMT940:
    ...
    def code86(cls, string86, bank_account, date, amount):
        ...
        if code == '':
            ...
            _ = re.search('//IBAN/(.*?(?=//))', string86)
            if _:
                payee = _.group(1) + ' - '

            _ = re.search('//NAME/(.*?(?=//))', string86)
            if _:
                payee = payee + _.group(1)

            string86 = re.sub(r"(/\D{3}/|/\D{4}/)",r"/\1", string86)
            _ = re.search('//REMI/(.*?(?=//))', string86)
            ...
            _ = re.search('IBAN:(.*?(?=BIC:))', string86)

            if _:
                payee = _.group(1).strip() + ' - '
                _ = re.search('NAAM:(.*?(?=OMSCHRIJVING:))', string86)

                if _:
                    payee = payee + _.group(1).strip()

            _ = re.search('OMSCHRIJVING:(.*$)', string86)

            if _:
                memo = (_.group(1)).strip()

        elif code == 'BEA  ':
            payee = 'PIN: '
            _ = re.search(r'(\d\d.\d\d\.\d\d/\d\d.\d\d\s.*$)', string86)

            if _:
                memo = _.group(1)

                _ = re.search(r'\d\d.\d\d\.\d\d/\d\d.\d\d\s(.*?(?=,))',
                              memo).group(1)
                if _:
                    payee = payee + _
        ...
        memo = re.sub('\s+',' ',m.strip())[:cls.cutoff]
        return payee, memo
